...
It is important to remember that the vertical growth was significantly different among treatments. Therefore, we should standardize the density of the new material to its growth. 
...
#now do the test on just the new growth sections
ctNew_test <- ct %>%
...
genos <- ct %>%
  ...
  bind_rows(ct %>%
             filter(genotype!='P-Lirman' & treatment=='LCO2') %>%
             mutate(genotype = "Everything Else"))

genos %>%
  group_by(genotype) %>%
  summarise(`standardized density` = mean(density.std, na.rm=T),
            sd = sd(density.std, na.rm = T)) %>%
  kbl(align = 'c',
    digits = 3,
    caption = "Comparison of P-Lrman to Everything Else in LCO2") %>%
  kable_classic() %>%
  row_spec(0, bold = T)
