...
```{r load CT data}
ctDataString <- list.files('~/Grad School/ULink 2022/LangdonCorals/materialData/', pattern = '.csv', full.names = T)
...
#800 thresholding w/ 7.5mm subsection of new growth
...
ctSub <- lapply(ctDataSubString, function(x) {
  read.csv(x, skip = 1) %>%
    filter(Material %in% c('NewGrowth','sub')) %>%
    select(2, 4,5, 9,14) %>%
    slice(1:3) %>%
    mutate(coral = case_when(
                    str_detect(x,
...
comp <- ctSub %>%
  filter(material == 'NewGrowth') %>%
  mutate(threshold=-800) %>%
  bind_rows(ct %>%
              filter(material == 'NewGrowth') %>%
              select(material:density,coral,treatment) %>%
              mutate(threshold = -250)) %>%
  pivot_longer(cols = c(volume,density),
              names_to = "metric")


comp %>%
  mutate(threshold = as.factor(threshold)) %>%
  ggplot(aes(threshold, value, fill=treatment)) +
  geom_boxplot() +
...
genos %>%
  ...
  theme_bw() +
  scale_fill_brewer(palette="Spectral") +
  labs(title = "Standardized Skeletal Density ~ Genotype",
       y = expression(paste("Standardized Skeletal Density (g ", cm^-3, day^-1, ")"))) 
