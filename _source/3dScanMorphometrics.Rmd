...
options(ggplot2.discrete.fill=wes_palette("Darjeeling1"),
        ggplot2.discrete.colour=wes_palette("Darjeeling1"))
...
## Morphometrics
```{r loadAcerData}
files <- list.files(path="~/Grad School/ULink 2022/3dScans/initial/FD/",
                    pattern = ".txt",
                    full.names = T)
...
lapply(files, function(x) {
  ...
  #extract metadata from filename
  id = stringr::str_sub(basename(x),1,4)
...
dat <- dat %>%
  #load SA,V, height
  left_join(read.csv("~/Grad School/ULink 2022/3dScans/initial/saVol_KielMarch22.csv") %>%
              select(tag, SA = `SA.cm2_corrected`,
                     V = `V.mm3_corrected`, SA_V, 
                     H = `H.cm_corrected`),
            by="tag") %>%
  #load FD calculated w/ Zawada et al (2019) functions
  left_join(read.csv("~/Grad School/ULink 2022/3dScans/initial/FDboxCounting.csv") %>%
              rename(FD_zawada=FD),
            by="tag") %>%
  #load convex hull metrics
  left_join(read.csv("~/Grad School/ULink 2022/3dScans/initial/convexHull_metrics.csv"),
            by="tag") %>%
...
#sphericity function
#sphericity = ratio of the SA of sphere w/ same volume of coral to SA of the coral
sphericity = function(vol, coralSA) {
  r = ((3*vol)/(4*pi))^(1/3)
  sphereSA = 4*pi*r^2
  return(sphereSA/coralSA)
}

#calculate shape metrics
dat <- dat %>%
  mutate(convexity = V/convexHull_Vol,
         sphericity = sphericity(V,SA),
         packing = SA/convexHull_SA)
...
mod_FDgeno <-FD_geno %>%
  ...
  mutate(
    # Apply ANOVA for each level of radii, for FD~Species
    anova = purrr::map(.x = data,
                .f = ~aov(FD~genotype,data = .x)),
    # Get results of ANOVA
    results = purrr::map(anova,tidy),
    tukey = purrr::map(anova, tukey_hsd)
    )
...
mod_FDgeno %>%
  ...
  count() %>%
  tidyr::pivot_wider(names_from="p.adj.signif",
              values_from = "n") %>%
  arrange(-`****`,-`***`,-`**`,-`*`,-ns) %>%
...
```{r shapeMorphometrics}
shapeMorphometrics <- dat %>%
  select(tag, genotype:treatment, FD=D11, FD_zawada, SA:H, convexity, sphericity, packing) %>%
  mutate(across(FD:packing, ~as.numeric(.x)),
         genotype=as.factor(genotype)) %>%
  tidyr::pivot_longer(FD:packing,
               names_to = "metric",
               values_to = "value")

capFig("Genotype-specific shape morphometrics")
shapeMorphometrics %>%
  filter(!metric %in% c('SA','V', 'H', 'FD_zawada')) %>%
  ggplot() +
  geom_boxplot(aes(x=value, y=fct_rev(genotype),
                  fill=genotype, color=genotype)) +
  theme_void() +
...
#PCA of morphometrics
PCADat <- shapeMorphometrics %>%
  filter(!metric %in% c('SA','V', 'H', 'FD_zawada')) %>% #remove FD_Zawada
  select(tag, genotype,metric, value) %>%
  tidyr::pivot_wider(names_from = "metric",
              values_from = "value") %>%
  #transform variables
  mutate(sphericity = log(sphericity/(1-sphericity)),
         packing = log10(packing),
         convexity = log(convexity/(1-convexity)))
  
pairs(PCADat %>% select(FD, convexity:packing))

rda.out <- vegan::rda(PCADat %>% select(FD:packing),
                      scale = TRUE, center = TRUE)

summary(rda.out)

BiodiversityR::PCAsignificance(rda.out)

fortify(rda.out, axes=1:2) %>%
  mutate(row_num = case_when(str_detect(label, 'sit') ~ row_number()-5,
                             TRUE ~ NA)) %>%
  left_join(PCADat %>%
              select(tag, genotype) %>%
               mutate(row_num = row_number()),
            by="row_num") %>%
  select(-row_num) -> pca_fort


ggplot(data = subset(pca_fort, score=="sites"),
       aes(PC1,PC2)) +
  geom_point(aes(color=genotype),
              
             alpha=0.2,
             show.legend = F) +
  # ggforce::geom_mark_ellipse(aes(colour=genotype), 
  #                         #fill=after_scale(alpha(colour, 0.8))), 
  #                     expand=0, size=0.2, show.legend=FALSE) +
   #choose one, polygon or 95% CI
  #polyggon method
  #95% confidence limits ellipse around the mean
  stat_conf_ellipse(aes(fill=genotype),
                    alpha = 0.7,
                    linetype = "dashed",
                    geom = "polygon",
                    level= 0.95) +
  ...
  geom_segment(data = subset(pca_fort, score=="species") %>%
                 left_join(data.frame(label = c('sphericity', 'convexity', 'packing', 'FD', 'SA_V'),
                           class = c(rep('no',2), rep('yes',2), 'no')),
                           by='label'),
               aes(x=0, y=0, xend=PC1/1.5, yend=PC2/1.5, colour=class),
               arrow = arrow(length=unit(0.03,"npc"),
                             type="closed"),
               size=0.8, show.legend=FALSE) +
  scale_colour_manual(values =  c("yes" = "#47775f", "no" = "darkgrey"),
                      name = "segment") +
  # geom_segment(data = subset(pca_fort, Score=="species"),
  #            aes(x=0, y=0, xend=-PC1/1.5, yend=-PC2/1.5),
  #            arrow = arrow(length=unit(0.03,"npc"),
  #                          type="closed"),
  #            color="darkgray", size=0.8, linetype=2) +
  geom_text(data = subset(pca_fort, score=="species") %>%
              mutate(label = case_when(label == 'SA_V' ~ 'SA:Volume', TRUE ~ label)),
            aes(label=label,
                x=PC1/1.5*1.1, y=PC2/1.5*1.1), nudge_x = .03) + 
           # family="Garamond", fontface="bold", size=8) +
  geom_vline(xintercept = c(0), color = "grey70", linetype = 3) +
  geom_hline(yintercept = c(0), color = "grey70", linetype = 3) +
  theme_bw() +
  #set limits
  ...
  theme(panel.grid = element_blank(),
        ...
        axis.title = element_text(size=18)) -> p

p
```
Only PC2 should be retained for analysis, % of variance explained > broken-stick percentage. Thus we only care how points are vertically distributed in the plot.
...
mod_morphCorrs <- morphCorrs %>%
  ...
  mutate(
    ...
    # Get results of ANOVA
    results = purrr::map(lm,summary),
    R = purrr::map_dbl(lm, ~ summary(.)$adj.r.squared))
...
This is in contrast to the lack of correlation in surface complexity and size for arborescent growth forms from Zawada *et al* (2019), which applies to *Acropora cervicornis* (Veron 2000). Zawada *et al* (2019) did, however, observe a positive correlation of FD and volume for corymbose growth forms, while they observed a negative correlation for massive growth forms. Zawada *et al* (2019) used a cubic box-counting method to determine FD rather than the spherical method employed here.


### Calculation of other Shape Variables


Zawada *et al* (2019) introduce 6 shape variables to quantitatively constrain coral morphology. These include,


  1. volume compactness
      * convexity - ratio of colony volume to convex hull volume
      * sphericity - ratio of SA of sphere with identical colony volume to SA of the colony volume
  2. Surface complexity
      * fractal dimension - same as described above; Zawada *et al* (2019) calculate with cubes
      * packing - ratio of colony SA to convex hull SA
  3. top-heaviness - classical engineering implementation of first moments
      * first moment of area - $\int\limits_{A_y}{xdA}$
      * first moment of volume - $\int\limits_{V_y}{xdV}$
      
      
With the goal of understanding how shape may affect ocean acidification sensitivity, I will focus on the first four shape variables as they may affect diffusion boundary layers (DBL) as water is slowed down by the complex morphology compared to bulk water flow. Top-heaviness is more applicable to determining a coral's mechanical stress as its vertical placement above the benthos such as dislodgement risk. Zawada *et al* (2019) similarly released publicly available code to calculate these variables, and I will employ their code here.


## Growth Analysis

An in depth analysis of treatments and growth <a href="https://patrickmkiel.com/notebook/research/ulink2022GrowthAnalysis?pass=ulink">can be viewed here.</a>
...
```{r gMorphCorrs}
FD_G <- totalGrowth %>%
  left_join(PCADat %>% select(tag, FD) %>% distinct(),
            by=c("tag"))
...
```{r BEMAnalysis}
FD_G <- totalGrowth %>%
  left_join(PCADat %>% select(tag, FD, packing) %>% distinct(),
            by=c("tag"))
...
FD_G_corrs <- FD_G %>%
  ...
  mutate(
    ...
    # Get results of ANOVA
    results = map(lm,summary),
    R = map_dbl(lm, ~ summary(.)$r.squared),
    p = map_dbl(lm, ~summary(.)$coefficients[2,4]),
    m = map_dbl(lm, ~summary(.)$coefficients[2,1]))

packing_G_corrs <- FD_G %>%
  nest(.by=treatment) %>% 
  mutate(
    # Apply ANOVA for each level of radii, for FD~Species
    lm = map(.x = data,
                .f = ~lm(G~packing,data = .x)),
    # Get results of ANOVA
    results = map(lm,summary),
    R = map_dbl(lm, ~ summary(.)$adj.r.squared),
    p = map_dbl(lm, ~summary(.)$coefficients[2,4]),
...
FD_G %>%
  ...
  facet_wrap(~treatment,
             labeller = label_parsed) +
  theme_bw() +
  labs(x = "Fractal Dimension") +
  theme(strip.text = element_text(face="bold", size = 12),
        text = element_text(family = "Garamond")) -> p

#packing is not as good, only 18%
FD_G %>%
  left_join(packing_G_corrs %>% select(treatment, R), by="treatment") %>%
  mutate(treatment = paste0(treatment, "~R^{2}==", signif(R,3))) %>%
  ggplot(aes(packing, G)) +
  geom_point(aes(color=genotype)) +
  geom_smooth(method="lm") +
  facet_wrap(~treatment,
             labeller = label_parsed) +
...
#now lets try w/ PCA, only retain 2nd PC
PCA_G <- totalGrowth %>%
  left_join(pca_fort %>% select(tag, PC1, PC2) %>% distinct(),
            by=c("tag"))
...
PCA_G %>%
  left_join(PCA_G_corrs %>% select(treatment, R), by="treatment") %>%
  mutate(treatment = paste0(treatment, "~R^{2}==", signif(R,3))) %>%
  ggplot(aes(PC2, G)) +
  geom_point(aes(color=genotype)) +
  geom_smooth(method="lm") +
...
# References
  ...
  1. Chan NCS, Wangpraseurt D, Kühl M, Connolly SR (2016) Flow and coral morphology control coral surface pH: Implications for the effects of ocean acidification. *Frontiers in Marine Science* 3:1-11.
  2. Chan WY, Eggins SM (2017) Calcification responses to diurnal variation in seawater carbonate chemistry by the coral Acropora formosa. *Coral Reefs* 36:763–772.
  3. Comeau S, Cornwall CE, Pupier CA, DeCarlo TM, Alessi C, Trehern R, McCulloch M (2019) Flow-driven micro-scale pH variability affects the physiology of corals and coralline algae under ocean acidification. *Scientific Reports* 9:1–12.
  4. Comeau S, Cornwall CE, Shlesinger T, Hoogenboom MO, Mana R, McCulloch MT, Rodolfo-Metalpa R (2022) pH variability at volcanic CO2 seeps regulates coral calcifying fluid chemistry. *Global Change Biology* 28(8):2751–2763. 
  ...
  6. Reichert J, Backes AR, Schubert P, Wilke T (2017) The power of 3D fractal dimensions for comparative shape and structural complexity analyses of irregularly shaped organisms. *Methods in Ecology and Evolution* 8(12):1650–1658. 
  ...
  8. Zawada KJA, Dornelas MA, Madin JS (2019) Quantifying coral morphology. *Coral Reefs* 38(6):1281–1292.
