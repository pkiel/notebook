...
`r capFig("Example photographs and 3D scans in Reichert et al. 2017")`
...
Given that the toolbox produces dilation radius from 1 <= R <= 20, you should be able to subset this data frame to just the integers 3 <= R <= 20 or all real numbers 3 <= R <= 20 to calculate and derive the same values reported in Reichert *et al.* (2017). So I'm going to do just that.
...
`r capFig("Theoretical 3D Application of the Bouligand-Minkowski method with spheres")``
...
dat %>%
  ...
  mutate(D8 = signif(as.numeric(D8,digits=7))) %>%
  kbl(caption = "Table 1: Comparison of Fractal Dimensions. I can replicate their FD.",
...
2. FD can generally discern between inter- and intra-specific, but its not perfect
...
2. from unique colony in nursery (a tree had >60 colonies all of one genotype)
3. minimal branching, 1 apical tip
